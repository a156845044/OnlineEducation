...
namespace XiaoWeiOnlineEducation.BLL
{
    ...
    public class Mod_Online_YearPlan_DetailBiz : Mod_Online_YearPlan_DetailBizBase
    {
        ...
        /// <param name="recordCount">总记录条数</param>
        /// <returns>信息查询列表</returns>
        public List<PlanDetailSearchEntity> GetMajorSearchbySchool(string planId, int yearId, string code, string typeId, string keyword, int pageSize, int pageNum, out int recordCount)
        {
            ...
            if (list.Count > 0)
            {
                ...
                var childQueryList = GetChildQueryList(planId, yearId, code, typeId, keyword, schoolId);
        ...
        /// <returns>信息查询列表</returns>
        public List<Mod_Online_YearPlan_DetailEntityExt> GetChildQueryList(string planId, int yearId, string code, string typeId, string keyword, string schoolIdStr)
        {
            string tempCommon = @"(SELECT TOP 1 child.CastScore FROM Mod_Online_YearPlan_Detail child INNER JOIN [Mod_Online_YearPlan] childPlan ON childPlan.PlanId=child.PlanId WHERE child.YearId={0} AND child.SchoolId =MOYD.SchoolId AND child.SchoolMajorId =MOYD.SchoolMajorId  AND child.AppTypeId =MOYD.AppTypeId ) ";
            ...
            SQL sql = SQL.Build(strSQL.ToString(), planId);
            return SqlMap<Mod_Online_YearPlan_DetailEntityExt>.ParseSql(sql).ToList();
        }
    }
}